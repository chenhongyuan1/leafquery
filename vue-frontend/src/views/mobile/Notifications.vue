    ...
    <div class="px-5 pt-6 flex-1 max-w-2xl mx-auto w-full">
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :class="isRead(announcement) ? 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800' : 'bg-blue-50/30 dark:bg-sky-500/10 border-blue-100 dark:border-sky-500/20'"
        >
          <div class="flex justify-between items-start mb-3 gap-3">
            <div class="flex items-center gap-2 min-w-0">
              <span v-if="!isRead(announcement)" class="w-2.5 h-2.5 bg-red-500 rounded-full shrink-0 shadow-sm shadow-red-500/50"></span>
              <span class="text-xs font-bold px-2 py-0.5 rounded border shrink-0" :class="typeColor(announcement.type)">
                {{ typeLabel(announcement.type) }}
              </span>
              <h2 class="font-bold leading-tight line-clamp-1 min-w-0" :class="isRead(announcement) ? 'text-slate-500 text-base' : 'text-slate-900 text-lg'">
                {{ announcement.title }}
              </h2>
            </div>
          ...
          <div class="text-[13px] line-clamp-2 leading-relaxed" :class="isRead(announcement) ? 'text-slate-400' : 'text-slate-500'" v-html="formatContent(announcement.content)"></div>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedItem" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div class="bg-white dark:bg-slate-900 w-full sm:w-[90%] sm:rounded-[2rem] rounded-t-[2rem] h-[85%] sm:h-auto overflow-hidden relative z-10 shadow-2xl dark:shadow-[0_24px_70px_rgba(2,6,23,0.55)] animate-slide-up flex flex-col transition-colors">
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div class="prose prose-slate prose-sm max-w-none text-slate-700 leading-loose whitespace-pre-wrap" v-html="formatContent(selectedItem.content)"></div>
        </div>
      </div>
    </div>
  </div>
