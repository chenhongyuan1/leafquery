...
- 项目当前定位为竞赛与研究原型，尚未完成大规模真实田间效果验证。
...
1. 删除配置文件和初始化 SQL 中的真实 API 密钥、私钥、令牌、数据库密码及示例账号密码。
2. 立即轮换曾经提交到 Git 历史中的所有凭据，仅删除当前文件内容并不能使旧凭据失效。
3. 使用环境变量或密钥管理服务注入敏感配置，并提交不含真实值的示例配置文件。
4. 为用户端和管理端补充服务端统一鉴权、密码哈希、权限校验和接口限流。
